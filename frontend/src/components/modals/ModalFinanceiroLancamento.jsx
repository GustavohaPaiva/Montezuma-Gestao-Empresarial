 ...
 * @param {(tipo: "entrada" | "saida", dados: object) => void} props.onSave
 * @param {string} [props.userTipo]
 * @param {string|number} [props.escritorioProprioId]
 * @param {string} [props.escritorioProprioNome]
 * @param {string} [props.visaoEscritorioAtual]
 * @param {boolean} [props.permitirEntrada] — se false, tipo fica travado em saída
 */
export default function ModalFinanceiroLancamento({
  isOpen,
  onClose,
  onSave,
  userTipo,
  ...
  const [tipo, setTipo] = useState(permitirEntrada ? "entrada" : "saida");
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setTipo(permitirEntrada ? "entrada" : "saida");
    }, 0);
    setTimeout(() => {
      setEscritorioLan(
        isSecretaria
  ...
  }, [isOpen, isSecretaria, visaoEscritorioAtual, permitirEntrada]);
          ...
          <div className="flex flex-col gap-3.5 overflow-y-auto px-5 py-5">
            <div className="flex flex-col gap-[5px]">
              ...
              {permitirEntrada ? (
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`h-11 rounded-xl border text-sm font-semibold transition-all ${
                      isEntrada
                        ? "border-emerald-500/40 bg-emerald-500/12 text-emerald-800 ring-2 ring-emerald-500/20"
                        : "border-border-primary/50 bg-[#FAFAFA] text-text-muted hover:bg-white"
                    }`}
                  >
                    Entrada
                  </button>
                  <button
                    ...
                    className={`h-11 rounded-xl border text-sm font-semibold transition-all ${
                      !isEntrada
                        ? "border-rose-500/40 bg-rose-500/12 text-rose-800 ring-2 ring-rose-500/20"
                        : "border-border-primary/50 bg-[#FAFAFA] text-text-muted hover:bg-white"
                    }`}
                  >
                    Saída
                ...
                <BaseSelect
                  ...
                  value="saida"
                  onChange={() => {}}
                  options={[{ value: "saida", label: "Saída" }]}
                />
              )}
            </div>
            ...
            <div className="flex flex-col gap-[5px]">
              ...
              <input
                type="text"
                placeholder={
                  isEntrada ? "Descrição da Entrada" : "Descrição da Saída"
                }
            ...
            >
              {isEntrada ? "Salvar Entrada" : "Salvar Saída"}
            </ButtonDefault>
          </div>
        </div>
      </div>
