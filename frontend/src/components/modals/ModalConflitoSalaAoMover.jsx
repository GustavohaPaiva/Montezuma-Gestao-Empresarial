...
export default function ModalConflitoSalaAoMover({
  isOpen,
  escritorioId,
  descricao,
  loading = false,
  ...
  const copy = homeDictionary.salaReunioes.conflitoAoMover;
  const temaClasse = temaEscritorio(escritorioId);

  return (
    <ModalPortal>
      <div
        className={`${temaClasse} fixed inset-0 z-[140] flex items-center justify-center bg-black/60 p-4 backdrop-blur-md`}
        role="dialog"
        aria-modal="true"
        aria-labelledby="conflito-sala-titulo"
      >
        <div className="animate-premium-reveal relative w-full max-w-lg overflow-hidden rounded-2xl border border-esc-border bg-esc-card shadow-[0_0_40px_-15px_var(--color-esc-destaque)]">
          ...
          <div className="flex items-center justify-between border-b border-esc-border px-5 py-4">
            <h3
              id="conflito-sala-titulo"
              className="text-base font-bold text-esc-text"
            >
              {copy.titulo}
            </h3>
            <button
          ...
          <div className="px-5 py-4">
            ...
            <p className="mt-2 text-xs text-esc-muted/80">
              {copy.hintCancelarSala}
            </p>
          </div>

          <div className="grid grid-cols-1 gap-2 border-t border-esc-border bg-esc-bg px-5 py-4 sm:grid-cols-2">
            <button
              type="button"
              onClick={onClose}
              disabled={loading}
