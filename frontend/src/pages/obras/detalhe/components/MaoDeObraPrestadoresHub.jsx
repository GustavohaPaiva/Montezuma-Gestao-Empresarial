...
import { SEM_PRESTADOR_ID } from "../utils/maoDeObraPorPrestador";

const joinClasses = (...classes) => classes.filter(Boolean).join(" ");

export default function MaoDeObraPrestadoresHub({
  prestadores = [],
  totaisObra = { contratado: 0, pago: 0, pendente: 0 },
  qtdPrestadores = 0,
  onSelectPrestador,
}) {
      ...
      <section className="w-full rounded-2xl border border-border-primary/35 bg-white p-4 shadow-sm ring-1 ring-slate-900/5 sm:p-5">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-4 text-left sm:grid-cols-4">
          ...
          <div className="min-w-0">
            ...
            <dd className="mt-1 text-lg font-semibold tabular-nums text-text-primary sm:text-xl">
              R$ {formatarMoeda(totaisObra.contratado)}
            </dd>
          </div>
          <div className="min-w-0">
            <dt className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wide text-slate-500">
        ...
        <div className="rounded-2xl border border-dashed border-border-primary/40 bg-slate-50/80 px-5 py-10 text-center">
          ...
          <p className="text-sm font-medium text-text-primary">
            {busca.trim()
              ...
              : "Nenhuma mão de obra lançada nesta obra"}
          </p>
          <p className="mt-1 text-xs text-text-muted">
            {busca.trim()
              ? "Tente outro nome ou limpe a busca."
              : "Quando houver lançamentos com prestador, eles aparecem aqui."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 sm:gap-4">
          {prestadoresFiltrados.map((p) => {
            const semPrestador = p.id === SEM_PRESTADOR_ID;
            const tudoPago = p.pendente <= 0 && p.contratado > 0;

            return (
              <button
                key={p.id}
                type="button"
                onClick={() => onSelectPrestador?.(p.id)}
                ...
                >
                  {tudoPago ? (
                    ...
                    <>
                      ...
                      Pendente: R$ {formatarMoeda(p.pendente)}
                    </>
                  ) : (
                    <>
